...
std::vector<Elite::Vector2> App_NavMeshGraph::FindPath(Elite::Vector2 startPos, Elite::Vector2 endPos)
{
	...
	const Triangle* pStartTriangle = nullptr;
	const Triangle* pEndTriangle = nullptr;
	...
	else if (pStartTriangle == pEndTriangle)
	{
	...
	NavGraphNode* startNode = new NavGraphNode(graphCopy.get()->GetNextFreeNodeIndex(), 0, startPos);
	graphCopy.get()->AddNode(startNode);
	for (int line : pStartTriangle->metaData.IndexLines)
	{
		if (m_pNavGraph->GetNodeIdxFromLineIdx(line) != invalid_node_index)
		{
			graphCopy.get()->AddConnection(new GraphConnection2D(m_pNavGraph->GetNodeIdxFromLineIdx(line), startNode->GetIndex(), Elite::Distance(startNode->GetPosition(), m_pNavGraph->GetNode(m_pNavGraph->GetNodeIdxFromLineIdx(line))->GetPosition())));
		}
		
	}
	//Create extra node for the End Node
	NavGraphNode* endNode = new NavGraphNode(graphCopy.get()->GetNextFreeNodeIndex(), 0, endPos);
	graphCopy.get()->AddNode(endNode);
	for (int line : pEndTriangle->metaData.IndexLines)
	{
		if (m_pNavGraph->GetNodeIdxFromLineIdx(line) != invalid_node_index)
		{
			graphCopy.get()->AddConnection(new GraphConnection2D(m_pNavGraph->GetNodeIdxFromLineIdx(line), endNode->GetIndex(), Elite::Distance(endNode->GetPosition(), m_pNavGraph->GetNode(m_pNavGraph->GetNodeIdxFromLineIdx(line))->GetPosition())));
		}
