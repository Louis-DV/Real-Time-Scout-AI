...
void App_AgarioGame::Start()
{
	...
	for (int i = 0; i < m_AmountOfAgents; i++)
	{
		...
		m_pStates.push_back(wanderState);

		FiniteStateMachine* simpleFSM = new FiniteStateMachine(wanderState,pB);
	...
	WanderState* wanderState = new WanderState();
	m_pStates.push_back(wanderState);
	SeekToFoodState* seekToFoodState = new SeekToFoodState();
	m_pStates.push_back(seekToFoodState);
	FleeFromAgentState* fleeFromAgentState = new FleeFromAgentState();
	m_pStates.push_back(fleeFromAgentState);
	...
	CloseToBiggerAgent* closeToBiggerAgentTrans = new CloseToBiggerAgent();
	m_pTransitions.push_back(closeToBiggerAgentTrans);
	SeekFoodToWander* seekFoodToWanderTrans = new SeekFoodToWander();
	m_pTransitions.push_back(seekFoodToWanderTrans);
	FleeToWander* fleeToWanderTrans = new FleeToWander();
	m_pTransitions.push_back(fleeToWanderTrans);

	FiniteStateMachine* complexFSM = new FiniteStateMachine(wanderState, pB);
	complexFSM->AddTransition(wanderState,seekToFoodState, closeToFoodTrans);
	complexFSM->AddTransition(wanderState,fleeFromAgentState, closeToBiggerAgentTrans);
	complexFSM->AddTransition(seekToFoodState,fleeFromAgentState, closeToBiggerAgentTrans);
	complexFSM->AddTransition(seekToFoodState, wanderState, seekFoodToWanderTrans);
	complexFSM->AddTransition(fleeFromAgentState, wanderState, fleeToWanderTrans);
