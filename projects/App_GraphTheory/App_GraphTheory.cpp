...
void App_GraphTheory::Update(float deltaTime)
{
	...
	if (path.size() != 0)
	{
		...
		{
			DEBUGRENDERER2D->DrawString(path[i]->GetPosition() + (path[i + 1]->GetPosition() - path[i]->GetPosition()) / 2 + Elite::Vector2{0,5}, to_string(i).c_str());
			//DEBUGRENDERER2D->DrawSegment(path[i]->GetPosition(), path[i+1]->GetPosition(), { 1,0,0 }, 0.001f);
		}
