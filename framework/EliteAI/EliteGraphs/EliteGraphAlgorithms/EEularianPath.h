...
namespace Elite
{
	...
	inline vector<T_NodeType*> EulerianPath<T_NodeType, T_ConnectionType>::FindPath(Eulerianity& eulerianity) const
	{
		...
		{
			pCurrentNode = graphCopy.get()->GetNode(0);
		}
		else
		{
			int i = 0;
			while (graphCopy.get()->GetNodeConnections(i).size()%2 == 0 && i < nrOfNodes)
			{
				i++;
			}
			pCurrentNode = graphCopy.get()->GetNode(i);
		}

		while (stack.size() > 0 || graphCopy.get()->GetNodeConnections(pCurrentNode->GetIndex()).size() > 0)
		{

			if (graphCopy.get()->GetNodeConnections(pCurrentNode->GetIndex()).size() == 0)
			{
			...
			else
			{
				...
				auto connection = graphCopy.get()->GetNodeConnections(pCurrentNode->GetIndex()).front();
				if (connection->GetFrom() == pCurrentNode->GetIndex())
				{
					pCurrentNode = graphCopy.get()->GetNode(connection->GetTo());
				}
				else
				{
					pCurrentNode = graphCopy.get()->GetNode(connection->GetFrom());
				}
				/*graphCopy.get()->GetNodeConnections(pCurrentNode->GetIndex())*/
				graphCopy.get()->RemoveConnection(connection);
